...
function resolveLink(node, link) {
    ...
    result +=
        '#' +
            encodeURIComponent(linkSegments
                .map((linkSegment) => {
                if (/^\(.*\)$/.test(linkSegment)) {
                    return linkSegment;
                }
...
function renderClassCard(node) {
    ...
    result += node.children
        ...
        if (x.kind === 2048) {
            r +=
                x.signatures
                    .map((signature) => renderFlags(x, 'inline') +
                    '<strong>' +
                    x.name +
                    '</strong>' +
                    render(signature) +
...
function render(node, style = 'inline') {
    ...
    if (node.type === 'reference') {
        ...
        if (!candidate) {
            candidate = getReflectionByName(name, parent, 4 | 128 | 256 | 4194304);
        }
        if (candidate) {
            ...
            if (candidate.kind === 16) {
                ...
                permalink.title = name;
            }
            return renderPermalink(permalink);
        }
        candidate = getReflectionByName(name, undefined, 4 | 128 | 256 | 4194304);
        if (candidate) {
                        ...
                        result +=
                            '<dt>' +
                                node.children
                                    ...
                                    const dd = renderFlags(x) + renderComment(x);
                                    return dt + '</dt><dd>' + dd;
                                })
                                    .join('</dd><dt>');
                        result += '</dd>';
...
function grok(src, options) {
    ...
    try {
        ...
        if (!apiName && mainPackage)
            apiName = mainPackage.name;
        if (apiName) {
            if (apiName.startsWith('"') && apiName.endsWith('"'))
                apiName = apiName.substring(1, apiName.length - 2);
            if (apiName.endsWith('.d'))
                apiName = apiName.substring(2);
        }
